...
# Open Streamlit in the default browser automatically
def open_browser():
    webbrowser.open("http://localhost:8501")

threading.Timer(1.5, open_browser).start()  # delay 1.5 seconds to start after server

# -------------------------------
# PAGE CONFIG
# -------------------------------
st.set_page_config(page_title="Financial Analytics Multi-Company", layout="wide")
st.title("📊 Financial Analytics & AI Forecasting System (Multi-Company)")
st.markdown("**Commercial Multi-Company Financial Prototype**")
...
# -------------------------------
try:
    cursor.execute("ALTER TABLE revenue ADD COLUMN company TEXT DEFAULT 'Company A'")
except sqlite3.OperationalError:
    pass  # Column already exists

try:
    cursor.execute("ALTER TABLE expenses ADD COLUMN company TEXT DEFAULT 'Company A'")
except sqlite3.OperationalError:
    pass  # Column already exists
conn.commit()

# -------------------------------
# INSERT SAMPLE DATA (MULTI-COMPANY)
# -------------------------------
...
if len(filtered_df) >= 2:
    ...
    st.metric(label=f"Predicted Revenue for {int(forecast['ds'].dt.year.iloc[-1])}", value=f"${predicted_revenue:,.2f}")
    if predicted_revenue > revenues[-1]:
        st.success("AI Insight: Revenue is expected to grow. Expansion recommended.")
    else:
        st.warning("AI Insight: Revenue may flatten. Cost optimization recommended.")
else:
